...
class drinkShop:
    ...
    def __init__(self, drinkName, drinkQuantity):
        self.drinkName = drinkName
        self.drinkQuantity = drinkQuantity
    def drink_name(self):
        return self.drinkName
    def totalPrice(self):
        if self.drinkName in self.itemDict:
            total = self.itemDict[self.drinkName] * self.drinkQuantity
        else: 
            return "not in stock!"
        return total

coca = drinkShop("Coca Cola", 5)
print(f"The total price of: {coca.drink_name()} is ${coca.totalPrice()}")
Peps = drinkShop("Pepsi", 10)
print(f"{Peps.drink_name()} is {Peps.totalPrice()}")
carabav = drinkShop("Carrabav", 6)
print(f"The total price of: {carabav.drink_name()} is ${carabav.totalPrice()}")